...
impl StatePartition{
    pub fn new(dfa: &DFA) -> StatePartition{
        ...
        for i in 0..dfa.states.len() {
            ...
            for j in 0..dfa.symbols_count{
                block.transitions.push(dfa.states[i].transitions[j as usize]);
            }
    ...
    pub fn union(&mut self, mut block_id_1: i32, mut block_id_2: i32){
        ...
        if self.blocks[block_id_1 as usize].label == UNKNOWN && self.blocks[block_id_2 as usize].label != UNKNOWN {
            self.blocks[block_id_1 as usize].label = self.blocks[block_id_2 as usize].label;
        } else if self.blocks[block_id_1 as usize].label == ACCEPTING && self.blocks[block_id_2 as usize].label == ACCEPTING {
            // Else, if both blocks are accepting, decrement accepting blocks count.
            self.accepting_blocks_count -= 1;
        } else if self.blocks[block_id_1 as usize].label == REJECTING && self.blocks[block_id_2 as usize].label == REJECTING {
        ...
        for i in 0..self.alphabet_size{
            ...
            if self.blocks[block_id_1 as usize].transitions[i] == -1 {
                self.blocks[block_id_1 as usize].transitions[i] = self.blocks[block_id_2 as usize].transitions[i];
    ...
    pub fn find(&mut self, state: i32) -> i32{
        ...
        while self.blocks[state_id as usize].root != state_id {
            ...
            if self.blocks[state_id as usize].root != self.blocks[self.blocks[state_id as usize].root as usize].root{
    ...
    pub fn merge_states(&mut self, state1: i32, state2: i32) -> bool{
        ...
        if (self.blocks[state1_id as usize].label == ACCEPTING && self.blocks[state2_id as usize].label == REJECTING) || (self.blocks[state1_id as usize].label == REJECTING && self.blocks[state2_id as usize].label == ACCEPTING) {
            return false;
        }
        ...
        for i in 0..self.alphabet_size{
            ...
            if self.blocks[state1_id as usize].transitions[i] == -1 || self.blocks[state2_id as usize].transitions[i] == -1{
                continue
            }
            // Else, merge resultant blocks.
            if !self.merge_states(self.blocks[state1_id as usize].transitions[i], self.blocks[state2_id as usize].transitions[i]){
                return false
            }
        }
