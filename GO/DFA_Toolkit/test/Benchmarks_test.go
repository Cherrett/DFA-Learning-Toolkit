...
// TestBenchmarkWindowedEDSM2 benchmarks the performance of the WindowedEDSMFromDataset2() function.
func TestBenchmarkWindowedEDSM2(t *testing.T) {
	// Random Seed.
	// rand.Seed(time.Now().UnixNano())
	...
	for i := 0; i < n; i++ {
		...
		resultantDFA, searchData := dfatoolkit.WindowedEDSMFromDataset2(trainingSet, targetSize*2, 2.0)
		accuracy := resultantDFA.Accuracy(testingSet)
...
func TestBenchmarkEDSM(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			// Decrement 1 from wait group.
			defer wg.Done()
			resultantDFAWindowed, _ = dfatoolkit.WindowedEDSM(APTA, targetSize*2, 2.0)
		}()
...
// TestBenchmarkEDSM2 benchmarks the performance of the GreedyEDSMFromDataset(), WindowedEDSMFromDataset()
...
func TestBenchmarkEDSM2(t *testing.T){
	...
	// Windowed 2
	winnersWindowed2 := 0
	accuraciesWindowed2 := util.NewMinMaxAvg()
	numberOfStatesWindowed2 := util.NewMinMaxAvg()
	durationWindowed2 := util.NewMinMaxAvg()
	mergesPerSecWindowed2 := util.NewMinMaxAvg()
	...
	for i := 0; i < n; i++ {
		...
		// Windowed 2
		resultantDFA, searchData = dfatoolkit.WindowedEDSMFromDataset2(trainingSet, targetSize*2, 2.0)
		durationWindowed2.Add(searchData.Duration.Seconds())
		mergesPerSecWindowed2.Add(searchData.AttemptedMergesPerSecond())
		//fmt.Printf("Windowed 2 Duration: %.2fs\n\n", searchData.Duration.Seconds())
		accuracy = resultantDFA.Accuracy(testingSet)
		accuraciesWindowed2.Add(accuracy)
		numberOfStatesWindowed2.Add(float64(len(resultantDFA.States)))
		if accuracy >= 0.99 {
			winnersWindowed2++
		}
	...
	successfulPercentageGreedy := float64(winnersGreedy) / float64(n)
	successfulPercentageWindowed := float64(winnersWindowed) / float64(n)
	successfulPercentageWindowed2 := float64(winnersWindowed2) / float64(n)
	successfulPercentageBlueFringe := float64(winnersBlueFringe) / float64(n)
	...
	fmt.Println("-----------------------------------------------------------------------------")
	fmt.Println("Windowed Search 2")
	fmt.Printf("Percentage of 0.99 <= Accuracy: %.2f%%\n", successfulPercentageWindowed2)
	fmt.Printf("Minimum Accuracy: %.2f Maximum Accuracy: %.2f Average Accuracy: %.2f\n", accuraciesWindowed2.Min(), accuraciesWindowed2.Max(), accuraciesWindowed2.Avg())
	fmt.Printf("Minimum States: %.2f Maximum States: %.2f Average States: %.2f\n", numberOfStatesWindowed2.Min(), numberOfStatesWindowed2.Max(), numberOfStatesWindowed2.Avg())
	fmt.Printf("Minimum Duration: %.2f Maximum Duration: %.2f Average Duration: %.2f\n", durationWindowed2.Min(), durationWindowed2.Max(), durationWindowed2.Avg())
	fmt.Printf("Minimum Merges/s: %.2f Maximum Merges/s: %.2f Average Merges/s: %.2f\n", mergesPerSecWindowed2.Min(), mergesPerSecWindowed2.Max(), mergesPerSecWindowed2.Avg())
	fmt.Println("-----------------------------------------------------------------------------")
