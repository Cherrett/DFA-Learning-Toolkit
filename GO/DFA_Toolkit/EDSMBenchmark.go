...
func BenchmarkEDSM(n int) {
	...
	for i := 0; i < n; i++ {
		...
		fmt.Printf("BENCHMARK %d/%d. Duration: %.2fs. Resultant DFA: %d states, Accuracy: %.2f. Average Accuracy: %.2f\n",
			i+1, n, time.Since(start).Seconds(), len(resultantDFA.States), accuracy, totalAccuracies/float64(i+1))
	}
